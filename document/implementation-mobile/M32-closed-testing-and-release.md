...
- The API is on a **free tier that sleeps** (web file 38). A reviewer who opens the app to a cold server sees M29's waking-up state — which is why that state exists, and why it must look deliberate rather than broken. Consider a keep-warm ping for the review window specifically; web file 38 documents the option.
...
6. **Keep the API up for review.** Coordinate the review window with the deployed server: it must be reachable and not mid-redeploy. If the free tier's sleep is a risk to a reviewer's first impression, enable the keep-warm option for the window.
...
6. Coordinate the review window with the deployed API, enabling keep-warm if the cold start risks the first impression. (~20 min)
