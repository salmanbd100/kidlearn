...
| 38 | `38-deployment-aws-docker.md` | Frontend on Vercel (two Hobby projects); both APIs on one EC2 `t4g.small` (`ap-south-1`) behind Caddy: prod on `api.kidlearn.net` + Supabase, dev on `api.dev.kidlearn.net` + a Postgres container. SSM secrets, Cloudflare DNS, per-env Cloudinary/Gemini — ~$13.75/month | §9, NFR-PERF-02, NFR-PERF-04 | 16, 29, 37, 37a | 8–9h | ⬜ Not started |
...
| `main` / `dev` divergence (no branch yet) | The parental PIN gate was removed on **`main`** (#48 — server enforcement, endpoints, UI, shared types, DB columns, FR-AUTH-04 retired) while **`dev`** moved the same files to new paths (#49) and still contains `require-pin-verified.ts`, `pin.ts`, `PinGate.tsx`, `PinPad.tsx` and `PinSetupScreen.tsx` — plus a design doc for removing them. A `dev` → `main` promotion will conflict across all of them or silently reintroduce the gate at its new paths. #48 reached `main` without going through `dev`, which is exactly what file 38a's `promotion-guard` exists to prevent and which does not exist yet. **Blocks file 38**, whose smoke test and spec §9 describe whichever parent flow actually ships. | Reviewing files 38/38a/39 | ⬜ Not started |
