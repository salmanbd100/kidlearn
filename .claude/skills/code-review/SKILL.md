...
- **`document/engineering-standards.md`** — code structure, naming, TypeScript, testing, enforcement rules
...
Read `document/engineering-standards.md` in full. Check every `[REVIEW]`-tagged rule against the diff. Focus especially on:

- **Semantic tokens** — any raw hex, CSS color literal, or Tailwind color class (`text-blue-500`, `bg-red-600`) used in a component instead of a semantic token (`text-foreground`, `bg-primary`). Rule: §2.
- **i18next** — any JSX string literal or template string that is user-visible and not routed through `i18next`. Rule: §5 (React), §10 (content voice).
- **`'use client'` placement** — boundary added higher in the tree than necessary; a Server Component converted to a Client Component when only a leaf needs interactivity. Rule: §5.
- **Component layer** — a component placed in the wrong `packages/ui` subdirectory (e.g. a kid-specific widget in `primitives/`, a pure utility in `kid/`). Rule: §2.
- **Route handler thickness** — business logic (Prisma queries, conditional branching, calculations) inside a route handler function body rather than a service. Rule: §6.
- **Zod missing** — a route handler that accepts a request body or params but has no Zod validation call before the service call. Rule: §6.
- **`new PrismaClient()`** — any direct instantiation in `apps/server` instead of using the `@kidlearn/db` singleton. Rule: §6.
- **Cross-package direction** — any import in `packages/*` that resolves to `apps/*`. Rule: §4.
- **Internal barrel files** — a new `index.ts` created inside a package at a path other than `src/index.ts` that re-exports from multiple sibling files. Rule: §4.
- **`enum` usage** — any TypeScript `enum` declaration. Rule: §3.
- **`as` cast without comment** — a type assertion (`as SomeType`) with no inline comment explaining why narrowing is not possible. Rule: §3.
- **`any`** — any explicit `any` type. Rule: §3.
...
Read `document/engineering-standards.md §3` and §4. Check the diff for:
...
Read `document/design.md §11` (the PR checklist) and `document/engineering-standards.md §2`. Check:
...
For each issue returned by any agent in Step 2, launch a parallel scoring agent. The scoring agent receives: the issue description, the relevant diff lines, and the rule from `document/engineering-standards.md` that was cited.
...
- **75** — Real issue, verified. Will affect functionality or directly violates a named rule in `document/engineering-standards.md`. The reviewer would block this.
...
1. **<one-line description>**
   ...
   Rule: `document/engineering-standards.md §<section>` — *"<exact quote of the rule>"*
...
- Quote the rule from `document/engineering-standards.md` verbatim for every standards violation.
