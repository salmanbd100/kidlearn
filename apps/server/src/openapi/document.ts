...
const DESCRIPTION = [
  ...
  "```",
  "",
  "`error.code` is stable and machine-readable — **branch on it, never on `message`**, which is a developer hint and may be reworded. It matters most behind `403`, where `FORBIDDEN` and `CONSENT_REQUIRED` mean two different next screens.",
  "",
  "### Authentication",
  "",
  ...
  "",
  "The usual sequence is: `GET /api/auth/google` → `POST /api/parent/consent` → `POST /api/children` → `POST /api/children/{id}/activate` → the `Content` endpoints.",
  "",
  "#### Native clients",
  "",
  ...
  "",
  "**There is no parental gate on the parent area.** A signed-in Google session reaches every parent endpoint, so a device left unlocked reaches the dashboard. Account deletion is two-step behind a single-use token; nothing else asks twice.",
  "",
  "### Content safety",
  "",
